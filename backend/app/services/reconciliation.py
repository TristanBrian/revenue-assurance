    ...
    try:
        ...
        if not anomalies_df.empty:
            ...
            for _, row in anomalies_df.iterrows():
                ...
                depot_val = row['depot'] if 'depot' in row and pd.notna(row['depot']) else None
...
def get_depot_alerts(depot_id: str, limit: int = 20) -> Dict:
    """
    Depot-scoped alert feed for depot_supervisor (routes/reconcile.py's
    require_permission("view_depot_alerts") always passes the CALLER'S OWN
    users.depot_id here — never a client-supplied value, so a depot
    supervisor can never see another depot's alerts by tampering a param).
    Reuses the same break-detection as run_reconciliation() (materiality=0,
    so nothing is dropped) and just filters the result down to one depot.
    """
    result = run_reconciliation(materiality=0)
    all_anomalies = result.get("anomalies", [])
    depot_anomalies = [a for a in all_anomalies if a.get("depot") == depot_id]
...
def get_depot_risk_summary() -> List[Dict]:
    """
    Per-depot leakage aggregation — backs the Heatmap page's Map view (depot
    markers + corridor coloring). Same risk_level bins as calculate_omc_risk
    (0-100k Low, 100k-1M Medium, 1M+ High) so a depot and an OMC mean the
    same thing when both say "High risk." materiality=0 so a depot with only
    small breaks still shows up (the map is about WHERE exposure is, not
    what clears the executive materiality bar).
    """
    result = run_reconciliation(materiality=0)
    ...
    bins = [0, 100000, 1000000, float("inf")]
    labels = ["Low", "Medium", "High"]
    stats["risk_level"] = pd.cut(stats["leakage_kes"], bins=bins, labels=labels).astype(str)
...
def get_omc_depot_map() -> List[Dict]:
    """
    Each OMC's total leakage/risk (same numbers as OMC Risk Profile — see
    calculate_omc_risk) plus the ONE depot they transact through the most.
    Backs the Heatmap Map view's OMC markers: real dispatch->depot data
    decides where a marker sits, rather than a fabricated address. An OMC
    with no depot on any of its dispatches is simply omitted.
    """
    result = run_reconciliation(materiality=0)
    anomalies = pd.DataFrame(result.get("anomalies", []))
    if anomalies.empty or "depot" not in anomalies.columns:
    ...
    bins = [0, 100000, 1000000, float("inf")]
    labels = ["Low", "Medium", "High"]
    omc_totals = anomalies.groupby("customer")["leakage_kes"].sum()
    omc_risk = pd.cut(omc_totals, bins=bins, labels=labels).astype(str)
    ...
    for _, row in primary.iterrows():
        ...
        out.append({
            "omc": omc,
            "depot_id": row["depot"],
            "leakage_kes": int(omc_totals.get(omc, 0)),
            "risk_level": omc_risk.get(omc, "Low"),
        })
    out.sort(key=lambda r: r["leakage_kes"], reverse=True)
    return out

# =============================================================================
# EXPOSURE VS RECOVERY TREND (daily, rolling window) — backs the manager
# dashboard's "Exposure vs recovery" chart. Both series come straight out of
# the live dispatches/invoices/payments tables, not a stored history: on
# each request, "exposure identified" re-derives from run_reconciliation_
# on_dataframes() (materiality=0, so nothing is dropped) bucketed by each
# anomaly's underlying dispatch date, and "recovered" sums real payment
# rows bucketed by payment date. All values in KES (the DB's native unit —
# nothing here converts currency, unlike the reference mockup this mirrors).
# =============================================================================
...
def get_exposure_recovery_trend(days: int = 30) -> List[Dict]:
    try:
        ...
        # Anchor the window to where dispatch activity is actually dense
        # rather than wall-clock now: the synthetic dataset's dispatch
        # generation trails off to a handful of stray future-dated rows
        # past its real cutoff, and anchoring on those would make every
        # "exposure identified" bucket in the requested window land on
        # empty days. The 99th percentile sits right at that real cutoff
        # and self-adjusts if the dataset is regenerated with new dates.
        ...
        # Exposure identified: every currently-open anomaly's leakage,
        # bucketed by its dispatch's date (materiality=0 keeps every break).
        ...
        if not anomalies.empty:
            ...
            exposure_by_day = (
                anomalies[anomalies["bucket_date"] >= window_start]
                .groupby("bucket_date")["leakage_kes"]
                .sum()
            )
        else:
            exposure_by_day = pd.Series(dtype=float)

        # Recovered: real payment inflows, bucketed by payment date.
        if not payments.empty and "date" in payments.columns and "value_kes" in payments.columns:
            ...
            recovered_by_day = (
                payments[payments["bucket_date"] >= window_start]
                .groupby("bucket_date")["value_kes"]
                .sum()
            )
        else:
        ...
        for d in date_index:
            trend.append({
                ...
                "exposure_identified_kes": int(exposure_by_day.get(d, 0)),
                "recovered_kes": int(recovered_by_day.get(d, 0)),
            })
        return trend
    except Exception as e:
        logger.error(f"❌ Exposure/recovery trend failed: {e}")
        raise