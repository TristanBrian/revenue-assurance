...
export default function InukaCaseDetailModal({ item, onClose }: { item: InukaRiskCase; onClose: () => void }) {
  ...
  const { user } = useAuth();
  const canAct = user?.permissions.includes("resolve_anomaly") ?? false;
  ...
  return (
    <AccessibleDialog label="Inuka case investigation" onClose={onClose}>
      <div className="flex max-h-[85dvh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-900" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 overflow-y-auto p-5">
          ...
          {canAct ? (<section className="mt-6 border-t border-zinc-200 pt-5 dark:border-zinc-800"><p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Record next action</p><div className="mt-3 grid gap-2 sm:grid-cols-[190px_1fr]"><select value={action} onChange={(event) => setAction(event.target.value)} className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950"><option value="acknowledge">Acknowledge</option><option value="request_evidence">Request evidence</option><option value="add_note">Add review note</option><option value="escalate">Escalate</option></select><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="What evidence was checked or requested?" className="min-h-20 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm dark:border-zinc-800 dark:bg-zinc-950" /></div><div className="mt-3 flex items-center justify-between gap-3"><span className="text-xs text-emerald-600">{message}</span><button onClick={submitAction} disabled={saving} className="rounded-lg bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">{saving ? "Saving…" : "Record action"}</button></div></section>) : <p className="mt-5 rounded-lg bg-muted p-3 text-sm text-muted-foreground">Read-only access · A Revenue Assurance analyst can record review actions.</p>}
