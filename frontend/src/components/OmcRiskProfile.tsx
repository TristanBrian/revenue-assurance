...
function DrilldownModal({ omc, onClose }: DrilldownModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto animate-fade-in">
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] transition-all">
        {/* Modal Header */}
        <div className="p-6 border-b border-zinc-800 flex justify-between items-start bg-zinc-950/40">
          <div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-indigo-400">OMC Profile Gantry Analysis</span>
              <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${riskBadgeClass(omc.risk_level)}`}>
                {omc.risk_level} Risk
              </span>
            </div>
            <h3 className="text-xl font-bold text-white mt-1">{omc.customer}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-zinc-950/40 border border-zinc-800/60 rounded-lg p-3 text-center">
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Total Identified Leakage</p>
              <p className="text-base font-bold text-rose-400 font-mono mt-1">{formatKes(omc.leakage_kes)}</p>
            </div>
            <div className="bg-zinc-950/40 border border-zinc-800/60 rounded-lg p-3 text-center">
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider">Active Anomalies</p>
              <p className="text-base font-bold text-white font-mono mt-1">{omc.anomaly_count}</p>
            </div>
          </div>

          {/* Timeline Section */}
          <div className="flex flex-col gap-3">
            <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Waybill Gantry Timeline</h4>
            ...
            {!loading && !error && anomalies.length > 0 && (
              <div className="relative border-l border-zinc-800 pl-4 py-2 flex flex-col gap-5">
                {anomalies.map((a, i) => (
                  <div key={`${a.dispatch_id}-${i}`} className="relative flex flex-col gap-1">
                    {/* Timeline Node Dot */}
                    <div className={`absolute -left-[21px] top-1.5 w-3 h-3 rounded-full border-2 border-zinc-900 ${
                      a.status === "Critical" ? "bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.6)]" :
                    ...
                    <div className="bg-zinc-950/20 border border-zinc-800/80 rounded-lg p-2.5 flex items-center justify-between gap-4 mt-1">
                      <div className="flex flex-col">
                        <span className="text-[9px] text-zinc-500 font-bold uppercase">Waybill ID</span>
                        <span className="text-xs text-zinc-300 font-mono mt-0.5">{a.dispatch_id}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-[9px] text-zinc-500 font-bold uppercase">Gap Leakage</span>
                        <span className="text-xs text-rose-400 font-bold font-mono mt-0.5">{formatKes(a.leakage_kes)}</span>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-950/30 flex justify-end">
          <button
            onClick={onClose}
            className="rounded bg-zinc-800 hover:bg-zinc-700 px-4 py-2 text-xs font-semibold text-zinc-300 hover:text-white transition-colors"
          >
            Close Details
          </button>
        </div>
...
export default function OmcRiskProfile({ profiles }: { profiles: OmcRiskProfileEntry[] }) {
  ...
  if (profiles.length === 0) {
    return (
      <div className="bg-zinc-900/30 border border-zinc-800 rounded-lg p-8 text-center">
        <p className="text-sm text-zinc-500">No OMC risk data available.</p>
      </div>
    );
  }
  ...
  return (
    <section className="flex flex-col gap-4">
      <h2 className="text-lg font-bold text-white">OMC Customer Risk Matrix</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
        {sorted.map((p) => (
          <div
            ...
            className="group cursor-pointer rounded-xl border border-zinc-800 bg-zinc-900/30 hover:bg-zinc-900/60 p-4 shadow-md transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 border-l-4 border-l-indigo-500 hover:border-l-indigo-400"
          >
            <div className="flex items-start justify-between gap-3">
              <h3 className="text-sm font-bold text-zinc-200 group-hover:text-white truncate flex-1">
                {p.customer}
              </h3>
            ...
            <div className="mt-3.5 border-t border-zinc-800/80 pt-2 flex items-center justify-end text-[10px] text-indigo-400 font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
