      ...
      <div className="w-full max-w-2xl rounded-3xl bg-slate-900 border border-slate-700/80 p-8 shadow-2xl space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center space-x-4 text-cyan-400">
            ...
            <div>
              <h2 className="text-2xl font-bold text-slate-100 tracking-tight">Cryptographic Report Verifier</h2>
              <p className="text-sm font-medium text-slate-400 mt-1">Verify file authenticity and tamper-evident SHA-256 signatures</p>
            </div>
          </div>
        ...
        <div className="space-y-3">
          <label className="block text-sm font-semibold text-slate-200">Select Exported Report File (.xlsx, .csv)</label>
          <input
            type="file"
            onChange={handleFileChange}
            accept=".xlsx,.csv"
            className="w-full text-sm text-slate-300 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-slate-800 file:text-slate-200 hover:file:bg-slate-700 cursor-pointer bg-slate-955/60 p-3 rounded-2xl border border-slate-800"
          />
        </div>
        ...
        {result && (
          <div className={`p-5 rounded-2xl border space-y-4 text-sm ${
            ...
            <div className="space-y-2 font-mono text-xs bg-slate-955/80 p-4 rounded-xl border border-slate-800 text-slate-300">
            ...
            {result.audit_match && (
              <div className="space-y-1.5 pt-2 border-t border-emerald-500/20 text-xs sm:text-sm text-slate-300">
                <div className="font-semibold text-slate-100">Audit Provenance Record Found:</div>
        ...
        <div className="flex items-center justify-end space-x-3 border-t border-slate-800 pt-5">
          ...
          <button
            ...
            className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 disabled:opacity-50 text-white text-sm font-semibold rounded-2xl transition flex items-center space-x-2 shadow-lg shadow-cyan-600/20"
          >
            {verifying ? (
