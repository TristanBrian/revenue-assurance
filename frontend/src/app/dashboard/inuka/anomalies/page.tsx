  ...
  return (
    <div className="flex flex-col gap-5 w-full max-w-[1700px] mx-auto">
      ...
      <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="border-b border-border bg-muted/40 text-[11px] uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Case</th><th className="px-4 py-3">Beneficiary / officer</th><th className="px-4 py-3">Evidence</th><th className="px-4 py-3">Amount at risk</th><th className="px-4 py-3">Score</th><th className="px-4 py-3">Severity</th></tr></thead><tbody className="divide-y divide-border">{loading ? <tr><td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">Loading assurance cases…</td></tr> : items.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center text-muted-foreground">No cases match the selected filters.</td></tr> : items.map((item) => <tr key={item.case_id} onClick={() => setSelectedCase(item)} className="cursor-pointer hover:bg-muted/20"><td className="px-4 py-3"><p className="font-semibold text-foreground">{item.title}</p><p className="mt-1 max-w-[260px] text-[11px] text-muted-foreground">{item.reason}</p><p className="mt-1 font-mono text-[10px] text-muted-foreground">{item.case_id}</p></td><td className="px-4 py-3">{item.beneficiary_id ? <Link href={`/dashboard/inuka/beneficiaries/${encodeURIComponent(item.beneficiary_id)}`} onClick={(event) => event.stopPropagation()} className="font-medium text-primary hover:underline">{item.beneficiary_id}</Link> : <p className="text-foreground">—</p>}<p className="mt-1 text-[11px] text-muted-foreground">Officer {item.officer_id || "unassigned"}</p><p className="text-[11px] text-muted-foreground">{item.pillar_id || "Pillar unassigned"}</p></td><td className="px-4 py-3"><span className="rounded-full bg-muted px-2 py-1 text-[11px] font-semibold text-muted-foreground">{item.confidence} confidence</span><p className="mt-2 text-[11px] text-muted-foreground">{item.source_records.length} source record{item.source_records.length === 1 ? "" : "s"}</p></td><td className="px-4 py-3 font-mono font-semibold text-status-critical">{formatKes(item.amount_at_risk)}</td><td className="px-4 py-3 font-mono font-semibold text-foreground">{item.risk_score}/100</td><td className="px-4 py-3"><span className={`rounded-full px-2 py-1 text-[11px] font-semibold ${item.severity === "Critical" ? "bg-status-critical-bg text-status-critical" : "bg-status-medium-bg text-status-medium"}`}>{item.severity}</span></td></tr>)}</tbody></table></div>
