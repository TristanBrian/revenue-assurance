...
export default function ExecutiveDashboardPage() {
  ...
  const [uploadedResult, setUploadedResult] = useState<ReconcileResult | null>(null);
  ...
  const currentMetrics = source === "database" ? metrics : uploadedResult?.metrics;
  const currentOmcs = source === "database" ? omcProfiles : uploadedResult?.omc_risk_profile;
    ...
    return (
      <div className="flex flex-col gap-6 max-w-5xl mx-auto text-zinc-100">
        ...
        <div className="bg-zinc-900/35 border border-zinc-900 rounded-xl p-6 flex flex-col gap-4 shadow-lg">
          <h2 className="text-base font-bold text-white">Welcome, {user.email}</h2>
          <p className="text-xs text-zinc-400 leading-relaxed">
            You are logged in as a <strong className="font-bold text-zinc-200">System Administrator</strong>. This account is designated for role management, user administration, and system-level configuration rather than financial operations.
          </p>
          <p className="text-xs text-indigo-400 font-semibold bg-indigo-950/20 border border-indigo-900/30 rounded p-3">
            💡 To inspect financial leakages, executive dashboards, or webhooks, please sign in with an operational account (e.g. Revenue Assurance or Manager).
          </p>
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto text-zinc-100">
        ...
        <div className="flex items-center gap-4">
          ...
          {user?.permissions.includes("upload_csv") && (
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-2 rounded border border-zinc-800 hover:border-zinc-700 bg-zinc-900/60 hover:bg-zinc-900 transition-colors text-xs font-semibold text-zinc-200"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      {source === "upload" && (
        <div className="flex items-center justify-between bg-indigo-950/20 border border-indigo-900/40 rounded-lg px-4 py-2.5 text-xs text-indigo-400">
          <span>Currently inspecting custom uploaded reconciliation datasets.</span>
          <button
            onClick={handleSwitchToLive}
            className="underline hover:text-indigo-300 font-bold ml-4"
          >
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {canViewOmcRisk ? (
              ...
              <div className="bg-zinc-900/35 border border-zinc-900 rounded-lg p-4 flex flex-col justify-between min-h-[96px]">
                <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">
                  Total Leakage
                </span>
                <span className="text-2xl font-bold tracking-tight text-white font-sans mt-1">
                  {formatKesCompact(totalLeakage)}
                </span>
                <span className="text-[10px] text-emerald-400 font-bold mt-1.5">
                  -14.8% vs last month
                </span>
              </div>
            ) : (
            ...
            {canViewOmcRisk ? (
              ...
              <div className="bg-zinc-900/35 border border-zinc-900 rounded-lg p-4 flex flex-col justify-between min-h-[96px]">
                ...
                <span className="text-[10px] text-amber-500 font-bold mt-1.5">
                  Under review
                </span>
              </div>
            ) : (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div className="lg:col-span-2">
              {canViewOmcRisk ? (
                <div className="bg-zinc-900/35 border border-zinc-900 rounded-xl p-5 flex flex-col gap-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <h2 className="text-sm font-bold text-white uppercase tracking-wider">
                        Top Leaking OMCs
                      </h2>
                      <span className="bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded text-[8px] font-bold uppercase">
                  ...
                  {sortedOmcs.length === 0 ? (
                    ...
                    <div className="flex flex-col divide-y divide-zinc-900">
                      {sortedOmcs.map((omc) => (
                        <div key={omc.customer} className="flex justify-between items-center py-3.5 first:pt-0 last:pb-0">
                          <div>
                            <h3 className="text-sm font-bold text-white">{omc.customer}</h3>
                            <p className="text-[10px] text-zinc-500 mt-0.5">
                              {omc.anomaly_count} anomalies detected across gantry logs
                            </p>
                          </div>
            ...
            <div className="bg-zinc-900/35 border border-zinc-900 rounded-xl p-5 flex flex-col gap-4 min-h-[220px] justify-between">
              <div>
                <h2 className="text-sm font-bold text-white uppercase tracking-wider">
                  Recent Uploads
                </h2>
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-lg p-6 shadow-2xl relative">
            <button
            ...
            <div className="mb-4">
              <h3 className="text-base font-bold text-white">Upload reconciliation datasets</h3>
              <p className="text-xs text-zinc-400">Drag or select a custom Gantry waybill CSV to run custom checks</p>
            </div>
